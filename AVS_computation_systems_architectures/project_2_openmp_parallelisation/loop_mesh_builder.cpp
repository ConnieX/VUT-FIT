...
unsigned LoopMeshBuilder::marchCubes(const ParametricScalarField &field){
    ...
    #pragma omp parallel for reduction(+:totalTriangles) default(none) shared(totalCubesCount, field) schedule(guided)
...
float LoopMeshBuilder::evaluateFieldAt(const Vec3_t<float> &pos, const ParametricScalarField &field){
    ...
    // 3. Finally take squa    // NOTE: This method is called from "buildCube(...)"!
