...
*      @name     move_mouse
*
*      @brief    This function moves with cursor in direction that is given by user,
*      			click with left or right mouse button or scrolls - all controlled by user.
*
*      @param    None      
*
*      @return   None
...
void move_mouse(void) 
{
    ...
    if(lwgpio_get_value(&special) == LWGPIO_VALUE_LOW && lwgpio_get_value(&left) == LWGPIO_VALUE_LOW){
    	g_mouse.rpt_buf[0] = 1;
    }
    else if(lwgpio_get_value(&special) == LWGPIO_VALUE_LOW && lwgpio_get_value(&right) == LWGPIO_VALUE_LOW){
		g_mouse.rpt_buf[0] = 2;
	}
    else if(lwgpio_get_value(&special) == LWGPIO_VALUE_LOW && lwgpio_get_value(&up) == LWGPIO_VALUE_LOW){
		g_mouse.rpt_buf[0] = 4;
		y -= 3;
	}
    else if(lwgpio_get_value(&special) == LWGPIO_VALUE_LOW && lwgpio_get_value(&down) == LWGPIO_VALUE_LOW){
		g_mouse.rpt_buf[0] = 4;
		y += 3;
	}
    else{
    	g_mouse.rpt_buf[0] = 0;
		if(lwgpio_get_value(&up) == LWGPIO_VALUE_LOW){
			y -= 6;
		}
		if(lwgpio_get_value(&down) == LWGPIO_VALUE_LOW){
			y += 6;
		}
		if(lwgpio_get_value(&right) == LWGPIO_VALUE_LOW){
			x += 6;
		}
		if(lwgpio_get_value(&left) == LWGPIO_VALUE_LOW){
			x -= 6;
		}
    }

    g_mouse.rpt_buf[1] = (uint8_t)(x);
    g_mouse.rpt_buf[2] = (uint8_t)(y);
...
void Main_Task( uint32_t param ){
	...
	if(!lwgpio_init(&up, BSP_BUTTON4, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE)){
		printf("\n Inicialization failed. \n ");
		_task_block();
	}
	lwgpio_set_functionality(&up, BSP_BUTTON4_MUX_GPIO);

	if(!lwgpio_init(&down, BSP_BUTTON2, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE)){
		printf("\n Inicialization failed. \n ");
		_task_block();
	}
	lwgpio_set_functionality(&down, BSP_BUTTON2_MUX_GPIO);

	if(!lwgpio_init(&right, BSP_BUTTON1, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE)){
		printf("\n Inicialization failed. \n ");
		_task_block();
	}
	lwgpio_set_functionality(&right, BSP_BUTTON1_MUX_GPIO);

	if(!lwgpio_init(&left, BSP_BUTTON3, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE)){
		printf("\n Inicialization failed. \n ");
		_task_block();
	}
	lwgpio_set_functionality(&left, BSP_BUTTON3_MUX_GPIO);

	if(!lwgpio_init(&special, BSP_BUTTON5, LWGPIO_DIR_INPUT, LWGPIO_VALUE_NOCHANGE)){
		printf("\n Inicialization failed. \n ");
		_task_block();
	}
	lwgpio_set_functionality(&special, BSP_BUTTON5_MUX_GPIO);
